...
if __name__ == '__main__':
    ...
    for key in keys:
        tmp_key = key
        tmp_key = aff.cleanup_affiliation_academic(tmp_key)
        tmp_key = aff.cleanup_affiliation_strip_chars(tmp_key)
        tmp_key = aff.cleanup_affiliation_suffix(tmp_key)
        if key in new_map:
            continue
        ...
        if key_lower_found:
            ...
            for tmp_item in tmp_val:
                tmp_item = aff.cleanup_affiliation_strip_chars(tmp_item) # remove white spaces
                tmp_item = aff.cleanup_affiliation_suffix(tmp_item) # remove suffixes
                if tmp_item == "":
                    continue
